...
#define DEBUG_START_STOP_PRINTS 1
...
void dynamicExecRun(process *processToRun, int quant){
	...
	if (pid==0){
		...
		int sleepResult = nanosleep(&sleepTime, &remaining);
		if(waitpid(-1, NULL, WNOHANG) > 0){
			printf("Child died, run taking care of it\n");
			processToRun->status=EXITED;
		}

		printf("sending kill\n");
		int killres = kill(pid, SIGSTOP);
		printf("kill sent");
		if(sleepResult==-1)
			printf("\nREMINING SECONDS AND NANOSECONDS: %ld %ld\n", remaining.tv_sec, remaining.tv_sec);

		double processEndTime = get_wtime();
		double elapsedTime = processEndTime - processStartTime;
		processToRun->timeUsed += elapsedTime;

		printf("KILL RES: %d SLEEP RESUILT : %d STATUS : %d\n", killres, sleepResult, processToRun->status);
		//if process didn't finish (handler didn't set exited),then set to stopped
		if (processToRun->status==RUNNING){
			printf("\nPROCESS WAS RUNNING\n");
			processToRun->status = STOPPED;
		...
		if(sleepResult==-1){
			if(processToRun->status!=EXITED){
				printf("\nPROCESS HAD FINISHED BUT VAR WAS NOT EXITED\n");
			}
			printf("\nSETTING TO EXITED\n");
			processToRun->status!=EXITED;
		}
...
void dynamicContinueRun(process *processToRun, int quant){
	...
	printf("sending stop\n");
	int killres= kill(processToRun->pid, SIGSTOP);
	printf("stop sent\n");
	// if(sleepResult==-1)
	// 	printf("\nREMAINING SECONDS AND NANOSECONDS: %ld %ld\n", remaining.tv_sec, remaining.tv_nsec);

	double processEndTime = get_wtime();
	double elapsedTime = processEndTime - processStartTime;
	processToRun->timeUsed += elapsedTime;

	printf("KILL RESULT: %d SLEEP RESUILT : %d STATUS : %d\n", killres, sleepResult, processToRun->status);
	//if process didn't finish (handler didn't set exited),then set to stopped
	if (processToRun->status==RUNNING){
		printf("\nPROCESS WAS RUNNING\n");
		processToRun->status = STOPPED;
...
{	
	...
	sigact.sa_flags = 0 | SA_NOCLDSTOP | SA_RESTART | SA_NODEFER;
	...
	if(DEBUG_INFO_PRINTS==1){
		...
		system("cd ../work && make clean >nul 2>nul");
	}
