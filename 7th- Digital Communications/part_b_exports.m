...
for i=0:3
    ...
    for iFig = 1:length(FigList)
      FigHandle = FigList(iFig);
      FigName   = strcat(FilePrefixes(i+1), num2str(get(FigHandle, 'Number'))); 
      saveas(FigHandle, fullfile(FolderName, strcat(FigName,'.png')));
    end
    close all;
end
...
function draw_figures(t, graph_prefix)
    ...
    for N=1:3
        for p=5:10
            ...
            msqerrors{N,p-4} = sumsqr(y_errors{N,p-4});
        end
    end
    ...
    hold off
    title(graph_prefix + "Original minus recreated signal for p=5, N=1,2,3")
    legend('Signal', 'N=1', 'N=2', 'N=3')
    xlabel('Sample')
    ylabel('value')
    ...
    hold off
    title(graph_prefix + "Original minus recreated signal for p=10, N=1,2,3")
    legend('Signal', 'N=1', 'N=2', 'N=3')
    xlabel('Sample')
    ylabel('value')
end
