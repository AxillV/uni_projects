...
void compute_knn_brute_force(double **xdata, double *q, int npat, int lpat, int knn, int *nn_x, double *nn_d)
{
	...
	//?(harris) omp slows it down, nnbs = 32, big overhead
	for (i = 0; i < knn; i++) {
		nn_x[i] = -1;
		nn_d[i] = 1e99-i;
	}
	
	//?(harris) max_i is reference to index of the max, so parallel messes it up
	max_d = compute_max_pos(nn_d, knn, &max_i);
	//#pragma omp parallel for private(new_d)
	for (i = 0; i < npat; i++) {
	...
	int temp_x, j;
	double temp_d;
	//?(harris): i runs from 0 to knn-1. j runs from 1 to i, so it is almost square matrix. wasted loops are not many 
	//?(harris) maybe int k=1, skip first line (negligible)
	#pragma omp parallel for //schedule(static)
	for (int k=0; k < (knn - 1)*(knn - 1); k++) {
		/* 
		...
		*/
		...
		if (j==0 || j>i) continue; //skip column0
...
double predict_value(int dim, int knn, double *xdata, double *ydata, double *point, double *dist)
{
	...
	double sum_v = 0.0;
	double sum_w = 0.0;
	double sum_vlocal = 0.0;
    double sum_wlocal = 0.0;

	#pragma omp parallel for reduction(+:sum_vlocal) reduction(+:sum_wlocal)
        for (i = 0; i < knn; i++) {
            //printf("  %f  ", sum_vlocal)) //TODO(?): debug
			double weight = 1/dist[i];  //weight is 1/dist[i]
            sum_vlocal += ydata[i] * weight;    
            sum_wlocal += weight;
...
/*double predict_value(int dim, int knn, double *xdata, double *ydata, double *point, double *dist)
{
    ...
    double sum_w = 0.0;
    for (i = 0; i < knn; i++) {
        sum_v += ydata[i]/dist[i];  //Weight is 1/dist[i]
        sum_w += 1/dist[i];
    }
